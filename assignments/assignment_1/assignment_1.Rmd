---
title: "assignment_1, Methods 3, 2025"
date: "2025-09-25"
...
2) Describe the data and construct extra variables from the existing variables  
    ...
    ii. describe what the following variables in the data frame contain, _trial.type_, _pas_, _trial_, _target.contrast_, _cue_, _task_, _target_type_, _rt.subj_, _rt.obj_, _obj.resp_, _subject_ and _correct_. (That means you can ignore the rest of the variables in your description). For each of them, indicate and argue for what `class` they should be classified into, e.g. _factor_, _numeric_ etc., and apply that class! (__Don't code variables as ordinal; if you think they are ordinal, just code them as a factor__)
...
1) Fit a multilevel model that models _correct_ as dependent on _task_ with a unique intercept for each _subject_  
    ...
    iv. finally, fit a model that models the interaction between _task_ and _pas_  and their main effects, still with a unique intercept for each _subject_  
    v. describe in your own words which model is the best in explaining the variance in accuracy, and based on your chosen model - write a short summary on what this says about accuracy as dependent on _pas_ and _task_. You should include at least one informative plot that shows the estimated probability of being correct for each of the levels of _task_ and _pas_ (and their interaction if you included that in your model).
...
2) Now fit a multilevel model that models a unique "slope" for _pas_ for each _subject_, $Z$, with the interaction between _pas_ and _task_ and their main effects being modelled on the first level, $X$
    ...
    vi. based on your chosen model - write a short summary on what this says about the distribution of ratings as dependent on _pas_ and _task_ and their interaction. You should include at least one informative plot that shows the estimated rate of events for each of the levels of _task_ and _pas_ (and their interaction if you included that in your model)  